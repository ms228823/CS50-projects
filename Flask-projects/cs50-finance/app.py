...
def history():
    ...
    cash_added_transactions = db.execute(
        "SELECT id, amount , transacted  FROM added_cash WHERE userid = ? ORDER BY transacted",
        session["user_id"],
    )
    return render_template(
        "history.html",
        user_transactions=user_transactions,
        cash_added_transactions=cash_added_transactions,
    )
...
def quote():
    ...
    if request.method == "POST":
        ...
        if not stock_quote:
            ...
            return render_template("quote.html", symbol=stock_quote)
...
def settings():
    ...
    return render_template(
        "settings.html", username=username[0]["username"], balance=balance[0]["cash"]
    )
...
def addcash():
    if request.method == "GET":
        ...
        if amount == None:
            ...
            # add added amount ti balance
            update = db.execute(
                "UPDATE users SET cash=:cash WHERE id=:userid ",
                cash=float(capital[0]["cash"]) + float(amount),
                userid=session["user_id"],
            )
            # add to added_cash history
            select_added_cash = db.execute("SELECT amount,userid FROM added_cash;")
            add_cash = db.execute(
                "INSERT INTO added_cash (amount,userid) VALUES (:amount,:userid);",
                amount=float(amount),
                userid=session["user_id"],
            )
